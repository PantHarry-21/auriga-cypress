/**
 * Sample Discarded — E2E Test Suite
 * URL  : /dashboard/samples/discarded
 * Role : admin
 */
import { test, expect } from '../global-setup';
import { stubStimulsoft, loginAs } from '../helpers/commands';

const URL = "/dashboard/samples/discarded";
const LAB = 'Arbro - Delhi';

test.describe("[MODULE-010] Sample Discarded", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/discarded/i);
    });
  ...
  test.describe('2. Search & Filter', () => {
    ...
    test('TC-006 search input is visible and accepts text', async ({ page }) => {
      const search = page.locator("input[placeholder=\"Search discarded samples...\"]").first();
