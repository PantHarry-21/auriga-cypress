/**
 * Micro Raw Data Enter — E2E Test Suite
...
test.describe("[MODULE-072] Micro Raw Data Enter", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/micro.{0,1}raw.{0,1}data/i);
    });
    ...
    test('TC-004 table has expected column headers', async ({ page }) => {
      ...
      expect(headers.some(h => h.includes("Batch Code No"))).toBe(true);
