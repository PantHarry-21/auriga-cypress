/**
 * Client PO — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-PURCHASE_CLIENT_PURCHASE_ORDER-PNE] Client PO — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("New Purchase Order")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('client-purchase-order');
    });
    ...
    test('TC-P04 list search accepts input (or is gated until filters applied)', async ({ page }) => {
      const search = page.locator('input[placeholder="Search by PO number or client name"]').first();
    ...
    test('TC-P05 "New Purchase Order" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("New Purchase Order")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="clientName"], textarea[name="clientName"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('input[name="clientName"], textarea[name="clientName"]').first();
      ...
      {
        const el = page.locator('input[name="poNo"], textarea[name="poNo"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="poAmount"], textarea[name="poAmount"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="remarks"], textarea[name="remarks"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="clientName"], textarea[name="clientName"]').first().isVisible({ timeout: 2000 }).catch(() => false);
