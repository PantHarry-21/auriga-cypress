/**
 * Mailer — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-MAIL_INBOX-PNE] Mailer — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("Compose")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P05 "Compose" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Compose")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="subject"], textarea[name="subject"]').first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('TC-P07 text form fields accept input', async ({ page }) => {
      ...
      {
        const el = page.locator('input[name="body"], textarea[name="body"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="subject"], textarea[name="subject"]').first().isVisible({ timeout: 2000 }).catch(() => false);
