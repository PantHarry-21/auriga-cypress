/**
 * Invoice Acknowledgement — E2E Test Suite
...
test.describe("[MODULE-090] Invoice Acknowledgement", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/invoice.{0,1}acknowledgement/i);
    });
    ...
    test('TC-004 table has expected column headers', async ({ page }) => {
      ...
      expect(headers.some(h => h.includes("Invoice No"))).toBe(true);
      expect(headers.some(h => h.includes("E-Invoice Date"))).toBe(true);
    });
   ...
   *   page.getByRole("button", { name: "To Upload" })
   *   page.getByRole("button", { name: "Uploaded" })
