/**
 * Archive Samples — E2E Test Suite
 * URL  : /dashboard/samples/archive
 * Role : admin
 */
import { test, expect } from '../global-setup';
import { stubStimulsoft, loginAs } from '../helpers/commands';

const URL = "/dashboard/samples/archive";
const LAB = 'Arbro - Delhi';

test.describe("[MODULE-009] Archive Samples", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/archive/i);
    });
  ...
  test.describe('2. Search & Filter', () => {
    ...
    test('TC-006 search input is visible and accepts text', async ({ page }) => {
      const search = page.locator("input[placeholder=\"Search by sample req no, company, product, batch…\"]").first();
   ...
   *   page.getByRole("button", { name: "Pending4496" })
   *   page.getByRole("button", { name: "Archived8199" })
