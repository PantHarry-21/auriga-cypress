...
const URL = "/dashboard/reports/dispatch-list";
const LAB = 'Arbro - Delhi';

test.describe("[MODULE-052] Dispatch List", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/dispatch.{0,1}list/i);
    });
  ...
  test.describe('2. Dispatch Actions', () => {
    ...
    test('TC-021 dispatch status radio filter is present', async ({ page }) => {
      expect(await page.locator('input[type="radio"][name="dispatchStatus"]').count()).toBeGreaterThan(0);
    });
  });
