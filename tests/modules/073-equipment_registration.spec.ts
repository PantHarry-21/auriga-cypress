/**
 * Equipment Registration — E2E Test Suite
...
test.describe("[MODULE-073] Equipment Registration", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/equipment/i);
    });
    ...
    test('TC-004 table has expected column headers', async ({ page }) => {
      ...
      expect(headers.some(h => h.includes("Equipment ID"))).toBe(true);
    });
