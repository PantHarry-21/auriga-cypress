/**
 * Block Sample — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
 * URL  : /dashboard/block-sample
 * Role : admin
...
const URL = '/dashboard/block-sample';
const LAB = 'Arbro - Delhi';

test.describe('[MODULE-BLOCK_SAMPLE-PNE] Block Sample — Positive/Negative/Edge', () => {
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('block-sample');
    });
    ...
    test('TC-P04 list search accepts input (or is gated until filters applied)', async ({ page }) => {
      const search = page.locator('input[placeholder="Search by Sample ID, Batch No, Product, or Company..."]').first();
