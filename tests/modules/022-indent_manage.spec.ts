/**
 * Indent Manage — E2E Test Suite
 * URL  : /dashboard/purchase/indent
 * Role : admin
 * Form : opened with "New Indent"
 */
...
test.describe("[MODULE-022] Indent Manage", () => {
  ...
  test.describe('3. Create Form', () => {
    ...
    test('TC-008 "New Indent" button is visible', async ({ page }) => {
      await expect(page.getByRole('button', { name: "New Indent" })).toBeVisible();
    });

    test('TC-009 form opens when "New Indent" is clicked', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("[name=\"PONo\"]").first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('TC-011 "PONo" field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("[name=\"PONo\"]").first()).toBeVisible({ timeout: 8000 });
      await page.locator("[name=\"PONo\"]").first().fill("automation test value");
      await page.locator("[name=\"PONo\"]").first().clear();
    });

    test('TC-012 "Heading" field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("[name=\"Heading\"]").first()).toBeVisible({ timeout: 8000 });
    });

    test('TC-013 "Search and select instrument..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("input[placeholder=\"Search and select instrument...\"]").first()).toBeVisible({ timeout: 8000 });
      await page.locator("input[placeholder=\"Search and select instrument...\"]").first().fill("automation test value");
      await page.locator("input[placeholder=\"Search and select instrument...\"]").first().clear();
    });

    test('TC-014 "Search and select company..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("input[placeholder=\"Search and select company...\"]").first()).toBeVisible({ timeout: 8000 });
      await page.locator("input[placeholder=\"Search and select company...\"]").first().fill("automation test value");
      await page.locator("input[placeholder=\"Search and select company...\"]").first().clear();
    });

    test('TC-015 "Generate Indent" button is present on form', async ({ page }) => {
      await page.getByRole('button', { name: "New Indent" }).click();
      await page.waitForTimeout(1500);
      await expect(page.getByRole('button', { name: "Generate Indent" }).first()).toBeVisible({ timeout: 8000 });
    });
