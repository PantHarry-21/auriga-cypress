/**
 * User Tracking Report — E2E Test Suite
...
const URL = "/dashboard/tracking";
const LAB = 'Arbro - Delhi';

test.describe("[MODULE-096] User Tracking Report", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/tracking/i);
    });
    ...
    test('TC-004 table has expected column headers', async ({ page }) => {
      ...
      expect(headers.some(h => h.includes("Tracking Date"))).toBe(true);
