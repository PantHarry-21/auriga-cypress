/**
 * Method Upload — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-METHOD_METHOD_UPLOAD-PNE] Method Upload — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("New Method Upload")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('method-upload');
    });
    ...
    test('TC-P05 "New Method Upload" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("New Method Upload")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="versionNo"], textarea[name="versionNo"]').first()).toBeVisible({ timeout: 10000 });
    });
    ...
    test('TC-P07 text form fields accept input', async ({ page }) => {
      ...
      {
        const el = page.locator('input[name="message"], textarea[name="message"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="versionNo"], textarea[name="versionNo"]').first().isVisible({ timeout: 2000 }).catch(() => false);
