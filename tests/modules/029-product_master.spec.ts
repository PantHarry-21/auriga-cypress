...
test.describe("[MODULE-029] Product Master", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/master/i);
    });
  ...
  test.describe('3. Create Form', () => {
    ...
    test('TC-008 "New Product" button is visible', async ({ page }) => {
      await expect(page.getByRole('button', { name: "New Product" })).toBeVisible();
    });

    test('TC-009 form opens when "New Product" is clicked', async ({ page }) => {
    ...
    test('TC-011 "Search brand name..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Product" }).click();
      await page.waitForTimeout(1500);
    ...
    test('TC-012 "Search generic name..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Product" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("input[placeholder=\"Search generic name...\"]").first()).toBeVisible({ timeout: 8000 });
      await page.locator("input[placeholder=\"Search generic name...\"]").first().fill("automation test value");
      await page.locator("input[placeholder=\"Search generic name...\"]").first().clear();
    });

    test('TC-013 "Search client name..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Product" }).click();
      await page.waitForTimeout(1500);
    ...
    test('TC-014 "Search matrix name..." field is visible and interactive', async ({ page }) => {
      await page.getByRole('button', { name: "New Product" }).click();
      await page.waitForTimeout(1500);
      await expect(page.locator("input[placeholder=\"Search matrix name...\"]").first()).toBeVisible({ timeout: 8000 });
      await page.locator("input[placeholder=\"Search matrix name...\"]").first().fill("automation test value");
      await page.locator("input[placeholder=\"Search matrix name...\"]").first().clear();
    });
