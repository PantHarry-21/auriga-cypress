/**
 * Upload Weight Slip — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-SAMPLES_WEIGHT_SLIP-PNE] Upload Weight Slip — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("Upload Weight Slip")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('weight-slip');
    });
    ...
    test('TC-P05 "Upload Weight Slip" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Upload Weight Slip")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('[placeholder="Enter sample number"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('[placeholder="Enter sample number"]').first();
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('[placeholder="Enter sample number"]').first().isVisible({ timeout: 2000 }).catch(() => false);
