...
test.describe("[MODULE-050] Upload Reports", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/upload.{0,1}reports/i);
    });
   ...
   *   page.getByRole("textbox", { name: "Enter report number to add to batch..." })
   *   page.getByRole("button", { name: "Add Report No" })
