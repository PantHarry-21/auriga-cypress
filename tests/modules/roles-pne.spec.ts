/**
 * All Roles — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
 * URL  : /dashboard/roles
 * Role : admin
...
test.describe('[MODULE-ROLES-PNE] All Roles — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("Create Role")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P05 "Create Role" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Create Role")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('[placeholder="e.g. Lab Manager, QA Analyst"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('[placeholder="e.g. Lab Manager, QA Analyst"]').first();
      ...
      {
        const el = page.locator('[placeholder="Why is this role needed?"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('[placeholder="e.g. Lab Manager, QA Analyst"]').first().isVisible({ timeout: 2000 }).catch(() => false);
