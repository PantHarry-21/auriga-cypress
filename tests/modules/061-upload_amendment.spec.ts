/**
 * Upload Amendment — E2E Test Suite
...
test.describe("[MODULE-061] Upload Amendment", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/upload.{0,1}amendment/i);
    });
    ...
    test('TC-004 table has expected column headers', async ({ page }) => {
      ...
      expect(headers.some(h => h.includes("Report No"))).toBe(true);
      expect(headers.some(h => h.includes("Remarks"))).toBe(true);
    });
   ...
   *   page.getByRole("textbox", { name: "Report No *" })
   *   page.getByRole("button", { name: "Upload File" })
   *   page.getByRole("textbox", { name: "Remarks" })
