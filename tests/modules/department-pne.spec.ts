/**
 * Department — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-DEPARTMENT-PNE] Department — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("New Department")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P04 list search accepts input (or is gated until filters applied)', async ({ page }) => {
      const search = page.locator('input[placeholder="Search department, location, requester…"]').first();
    ...
    test('TC-P05 "New Department" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("New Department")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="deptName"], textarea[name="deptName"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('input[name="deptName"], textarea[name="deptName"]').first();
      ...
      {
        const el = page.locator('input[name="subDepartmentName"], textarea[name="subDepartmentName"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="location"], textarea[name="location"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="reason"], textarea[name="reason"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="deptName"], textarea[name="deptName"]').first().isVisible({ timeout: 2000 }).catch(() => false);
