/**
 * Upload Amendment — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
 * URL  : /dashboard/reports/upload-amendment
 * Role : admin
...
const URL = '/dashboard/reports/upload-amendment';
const LAB = 'Arbro - Delhi';

test.describe('[MODULE-REPORTS_UPLOAD_AMENDMENT-PNE] Upload Amendment — Positive/Negative/Edge', () => {
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('upload-amendment');
    });
    ...
    test('TC-P04 list search accepts input (or is gated until filters applied)', async ({ page }) => {
      const search = page.locator('input[placeholder="Search by report number, remarks, or employee name..."]').first();
    ...
    test('TC-P05 "Upload" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Upload")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('[placeholder="Enter remarks"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('[placeholder="Enter remarks"]').first();
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('[placeholder="Enter remarks"]').first().isVisible({ timeout: 2000 }).catch(() => false);
