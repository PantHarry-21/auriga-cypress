...
test.describe("[MODULE-064] Archive Report", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/archive.{0,1}report/i);
    });
   ...
   *   page.getByRole("button", { name: "Pending2,727" })
   *   page.getByRole("button", { name: "Archived4,901" })
