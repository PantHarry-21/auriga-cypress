/**
 * TRF Links — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-SAMPLES_TRF_LINKS-PNE] TRF Links — Positive/Negative/Edge', () => {
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('trf-links');
    });
    ...
    test('TC-P05 "Create TRF Link" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Create TRF Link")').first()).toBeVisible({ timeout: 15000 });
    });
  });
