...
test.describe("[MODULE-004] Allot Sample", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/allot.{0,1}sample/i);
    });
   ...
   *   page.getByRole("button", { name: "To Receive 0" })
   *   page.getByRole("button", { name: "Received 0" })
