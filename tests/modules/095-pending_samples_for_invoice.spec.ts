/**
 * Pending Samples for Invoice — E2E Test Suite
...
test.describe("[MODULE-095] Pending Samples for Invoice", () => {
  ...
  test.describe('1. Page Load', () => {
    ...
    test('TC-002 page URL contains expected path segment', async ({ page }) => {
      ...
      expect(page.url()).toMatch(/invoice.{0,1}pending.{0,1}samples/i);
    });
...
}); // describe Pending Samples for Invoice
