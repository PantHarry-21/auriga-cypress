/**
 * CRM Working Standard — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-CRM_WORKING_STANDARD-PNE] CRM Working Standard — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("New CRM")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('crm-working-standard');
    });
    ...
    test('TC-P05 "New CRM" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("New CRM")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="productName"], textarea[name="productName"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('input[name="productName"], textarea[name="productName"]').first();
      ...
      {
        const el = page.locator('input[name="codeNo"], textarea[name="codeNo"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="versionNo"], textarea[name="versionNo"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="storageCondition"], textarea[name="storageCondition"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="productName"], textarea[name="productName"]').first().isVisible({ timeout: 2000 }).catch(() => false);
      expect((await errs.count()) > 0 || stillOpen).toBe(true);
    });

    test('TC-N02 number field "noOfVials" rejects alphabetic input', async ({ page }) => {
      await openForm(page);
      const el = page.locator('input[name="noOfVials"]').first();
      if (!(await el.isVisible({ timeout: 4000 }).catch(() => false))) { test.skip(); return; }
