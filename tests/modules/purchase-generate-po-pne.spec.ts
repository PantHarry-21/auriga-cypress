/**
 * Generate PO — Positive / Negative / Edge Case Suite (auto-generated from live prod crawl 2026-07-20)
...
test.describe('[MODULE-PURCHASE_GENERATE_PO-PNE] Generate PO — Positive/Negative/Edge', () => {
  ...
  async function openForm(page) {
    await page.locator('button:has-text("Generate PO")').first().click({ force: true });
  ...
  test.describe('Positive', () => {
    ...
    test('TC-P02 URL reflects the module route', async ({ page }) => {
      expect(page.url()).toContain('generate-po');
    });
    ...
    test('TC-P05 "Generate PO" action is present', async ({ page }) => {
      await expect(page.locator('button:has-text("Generate PO")').first()).toBeVisible({ timeout: 15000 });
    });

    test('TC-P06 opening the form reveals its fields', async ({ page }) => {
      await openForm(page);
      await expect(page.locator('input[name="Address"], textarea[name="Address"]').first()).toBeVisible({ timeout: 10000 });
    });

    test('TC-P07 text form fields accept input', async ({ page }) => {
      await openForm(page);
      {
        const el = page.locator('input[name="Address"], textarea[name="Address"]').first();
      ...
      {
        const el = page.locator('input[name="QuotationNo"], textarea[name="QuotationNo"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="BankDetails"], textarea[name="BankDetails"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
      {
        const el = page.locator('input[name="Heading"], textarea[name="Heading"]').first();
        if (await el.isVisible({ timeout: 4000 }).catch(() => false) && await el.isEnabled().catch(() => false)) {
          await el.fill('AUTOQA'); await expect(el).toHaveValue(/AUTOQA/, { timeout: 4000 });
        }
      }
  ...
  test.describe('Negative', () => {
    test('TC-N01 submitting the empty form is blocked or shows validation', async ({ page }) => {
      ...
      const stillOpen = await page.locator('input[name="Address"], textarea[name="Address"]').first().isVisible({ timeout: 2000 }).catch(() => false);
